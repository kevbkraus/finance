...
winners_df = pd.DataFrame(columns=['Company name', 'Share price', 'Analyst target', 'PE', 'Debt rating', 'Min VPS', 'Avg VPS', 'Max VPS'])
losers_df = pd.DataFrame(columns=['Company name', 'Share price', 'Analyst target', 'PE', 'Debt rating', 'reason for failure']
...
for i in range(0,100): # Process 100 random stocks per run of this code
    ...
    if response['result'] == 'failure':
        ...
        losers_df.loc[lose_idx, 'response for failure'] = response['response for failure']
        lose_idx = lose_idx + 1
        print('Failed to process ', symbol)        

        if lose_idx > 9: # Once we have assembled 10 losers, commit the df to the excel file to prevent unexpected data loss
    ...
    if response['result'] == 'failure':
        ...
        if win_idx > 9: # Once we have assembled 10 winners, commit the df to the excel file to prevent unexpected data loss
