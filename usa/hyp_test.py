...
parser.add_argument('-r', '--sample_size', help='Sample size. If not supplied, all pertinent stocks will be taken into consideration')
...
help_text = """ (Hypothesis number) Hypothesis
                ...
                (2) Price vs. EBITDA
                (3) Return per risk vs. % held by instituitional investors
                (4) Best Return per risk stocks vs. SP500 """
parser.add_argument('h', 'hyp_num', help=help_text, type=int)
...
if args.hyp_num == 1:
    ...
    print(' -----------------------------------------------------------------------------------------')
    xy_df = pd.DataFrame(columns=['PE', 'net margin'])
    for symbol in sublist['Symbol'].values:
        ...
        if args.begin_year:
            inc_stmt_annual = inc_stmt_annual.loc[inc_stmt_annual.index > args.begin_year
        ...
        idx = list(map(lambda x: prices_yearly.index.get_loc(x, method='nearest'), inc_stmt_annual.index.values)) # Because dates don't match exactly between
                                                                                                                  # price dataframe and income statement data
                                                                                                                  # we have to find the nearest dates
        samples_df = pd.DataFrame({'PE':prices_yearly.iloc[idx].values/inc_stmt_annual['eps'].values, 'net margin':inc_stmt_annual['netIncomeRatio'].values})
        xy_df = xy_df.append(samples_df)
