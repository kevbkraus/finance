...
count = 0
risk_return = pd.DataFrame(columns=['symbol', 'sector', 'mean annual return', 'std annual return'])
for symbol, isector in tqdm(zip(symbols,sectors), desc='Symbols processed'):
    ...
    risk_return = risk_return.append({'symbol':symbol, 'sector': isector, 'mean annual return':mean_ret, 'std annual return':std_ret}, ignore_index=True)
...
risk_return['return per risk'] = risk_return['mean annual return']/risk_return['std annual return']
...
if not args.sector:
    ...
    sns.catplot(y='sector', x='std annual return', data=risk_return, kind='box', orient='h')
